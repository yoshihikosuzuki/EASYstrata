...
insert the name of the species to homogenize the chromosome ID:

```
sed 's/^>/>stickleback_/g' ../stickleback_v5.0.1_assembly.fa > stickleback.fa
zcat stickleback_v5_ensembl_genes.gff3.gz stickleback_v5_maker_genes_chrY.gff3.gz \
    |sed 's/^chr/stickleback_chr/' > stickleback.gff3
```

#run busco on the genome and on the protein prediction to assess the quality of the data:
 ...
 ***** Results: *****
        ...
        C:98.0%[S:97.3%,D:0.7%],F:1.2%,M:0.8%,n:3640,E:2.7%        
        3566    Complete BUSCOs (C)     (of which 95 contain internal stop codons)   
        3540    Complete and single-copy BUSCOs (S)        
...
#then run busco on the protein file :
...
**Important note:** 
actually, the Y gff comes from another annotation tool (maker) and seem not
really compatible with the rest of the annotation.  

Therefore we use [miniprot](https://github.com/lh3/miniprot) to transfer the stickleback X based gene to the Y chromosome.
...
- number of single copy orthologs from the N0.tsv file from Orthofinder : 323 


- here is the dS values obtained from codeml in pairwise mode:


- median dS from codeml : 0.0490
- median dS from yn00 :   0.04380
